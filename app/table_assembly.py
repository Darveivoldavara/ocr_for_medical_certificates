...
def assembly(lst):
    ...
    while i < len(lst):
        ...
        if (
            re.match(
                r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*\d{2}[\W_]*(\d{2}|\d{4})[\W_]*$", lst[i]
            )
            and i + 2 < len(lst)
            and re.match(r"^\w{1,2}[\W_]?\w{,2}[\W_]?\w{,4}[\W_]?$", lst[i + 1])
        ):
            ...
            if re.match(r"^\w{1,2}[\W_]?\w{,2}[\W_]?\w{,4}[\W_]$", lst[i + 1]):
                donation_type = re.findall(r"\w{1,2}[\W_]?\w{,2}", lst[i + 1])[
                    0
                ].lower()
                is_paid = re.findall(r"[\W_]?\w{,4}[\W_]$", lst[i + 1])[0].lower()
                i += 2
            ...
            if re.match(r"^\w{1,2}[\W_]?\w{,2}[\W_]?\w{,4}[\W_]$", lst[i + 2]):
                donation_type = re.findall(r"\w{1,2}[\W_]?\w{,2}", lst[i + 2])[
                    0
                ].lower()
                is_paid = re.findall(r"[\W_]?\w{,4}[\W_]$", lst[i + 2])[0].lower()
                i += 3
            ...
            re.match(r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*\d{2}[\W_]*$", lst[i])
            and i + 3 < len(lst)
            ...
            if re.match(r"^\w{1,2}[\W_]?\w{,2}[\W_]?\w{,4}[\W_]$", lst[i + 3]):
                donation_type = re.findall(r"\w{1,2}[\W_]?\w{,2}", lst[i + 3])[
                    0
                ].lower()
                is_paid = re.findall(r"[\W_]?\w{,4}[\W_]$", lst[i + 3])[0].lower()
                i += 4
        ...
        if donation_type:
            ...
            elif "k" in donation_type or "w" in donation_type or "p" in donation_type:
                donation_type = "blood"
            elif "n" in donation_type or "m" in donation_type or "v" in donation_type:
                donation_type = "plasma"
            elif (
                re.search(r"[a-z]", donation_type)
        ...
        if is_paid:
            if 0 < len(re.findall(r"[a-z0-9]", is_paid)) < 3:
                is_paid = "free"
            elif 2 < len(re.findall(r"[a-z0-9]", is_paid)) < 5 and (
                "n" in is_paid or "t" in is_paid
            ):
                is_paid = "payed"
            elif (
                len(re.findall(r"[a-z0-9]", is_paid)) > 14
        ...
        if (
            (
                re.match(r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*$", str(is_paid))
                or re.match(r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*\d{2}[\W_]*$", str(is_paid))
            )
            and i + 1 < len(lst)
            and (
                re.match(r"^[\W_]*\d{2}[\W_]*(\d{2}|\d{4})[\W_]*$", lst[i])
                or re.match(r"^[\W_]*(\d{2}|\d{4})[\W_]*$", lst[i])
            )
        ):
            i -= 1
            continue
        ...
        if (
            (
                ...
                or re.match(r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*$", str(donation_type))
                or re.match(
                    r"^[^\w\s\(\)]*\d{2}[^\w\s\(\)]*\d{2}[\W_]*$", str(donation_type)
                )
            )
            and i + 1 < len(lst)
            and (
                re.match(r"^\w{1,2}[\W_]?\w{,2}[\W_]?\w{,4}[\W_]?$", str(is_paid))
                or re.match(r"^[\W_]*\d{2}[\W_]*(\d{2}|\d{4})[\W_]*$", str(is_paid))
                or re.match(r"^[\W_]*(\d{2}|\d{4})[\W_]*$", str(is_paid))
            )
        ):
            i -= 2
            continue
    ...
    for idx, row in invalid_rows.iterrows():
        df = pd.concat([df.iloc[:idx], row.to_frame().T, df.iloc[idx:]]).reset_index(
            drop=True
        )

    df = df.fillna(value={"is_paid": "free", "donation_type": "blood"})
